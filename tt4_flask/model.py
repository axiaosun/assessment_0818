...
def calculateHoldings_byTicker(username):
	connection = sqlite3.connect(database, check_same_thread = False)
	cursor = connection.cursor()
	query = 'SELECT ticker_symbol, sum(num_shares*last_price) FROM holdings WHERE username = "{}" GROUP BY ticker_symbol;'.format(username)
...
if __name__ == '__main__':
	print(calculateHoldings_byTicker('john'))
	print(calculateHoldings('john'))
	print(get_user_balance('john'))